...
IPERF_ARG=" -c ""$ADDRESS"" -p ""$PORT"" --get-server-output -M ""$MSS"" ""$DYN_ARG"
printf "sudo ip netns exec shila-egress iperf3 %s\n" "$IPERF_ARG"
sudo ip netns exec shila-egress iperf3 "$IPERF_ARG" >> "$LOG_FILE" 2>> "$ERR_FILE"
