...
func runClient(address string) error {
	...
	parsedAddr, err := appnet.ResolveUDPAddr(address)
	if err != nil {
		return err
	}

	paths, err := appnet.QueryPaths(parsedAddr.IA)
	if err != nil {
		return err
	}

	for _, path := range paths {
		fmt.Print(path.Expiry())
	}

	appnet.SetPath(parsedAddr, paths[3])

	conn, err := appnet.DialAddr(parsedAddr)
	if err != nil {
		return err
	}
	defer conn.Close()
